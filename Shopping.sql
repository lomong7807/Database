...
INSERT INTO `Products` (`prodNo`, `cateNo`, `sellerNo`, `prodName`, `prodStock`, `prodPrice`, `prodSold`)
VALUES (130101, 13, 10006, '[ANF] 식스프리 강아지 사료', 58, 56000, 142);
INSERT INTO `Products` (`prodNo`, `cateNo`, `sellerNo`, `prodName`, `prodStock`, `prodPrice`, `prodSold`)
VALUES (130112, 13, 10006, '중대형 사계절 강아지옷', 120, 15000, 80);
INSERT INTO `Products` (`prodNo`, `cateNo`, `sellerNo`, `prodName`, `prodPrice`, `prodSold`)
VALUES (141001, 14, 10001, '라떼 2인 소파/방수 패브릭', 320000, 42);
INSERT INTO `Products` VALUES (170115, 17, 10007, '지포스 3080 그래픽카드', 28, 900000, 12, 12);
INSERT INTO `Products` (`prodNo`, `cateNo`, `sellerNo`, `prodName`, `prodStock`, `prodPrice`, `prodSold`)
VALUES (160103, 16, 10009, '치닝디핑 33BR 철봉', 32, 120000, 28);
...
INSERT INTO `OrderItems` VALUES (11, '22010510021', 100101, 25000, 21, 1);
...
#문제 1
# 모든 장바구니 내역에서 고객명, 상품명, 상품수량을 조회하시오. 단 상품수량 2건 이상만 조회 할 것
SELECT
		c.userName,
		b.prodName,
		a.cartProdCount
...
#문제 2
#모든 상품 내역에서 상품번호, 상품카테고리명, 상품명, 상품가격, 판매자이름, 판매자 연락처를 조회하시오.
SELECT
		a.prodNo,
		b.cateName,
		a.prodName,
...
#문제 3
#모든 고객의 아이디, 이름, 휴대폰, 현재포인트, 적립포인트 합을 조회하시오. 단 적립포인트 내역이 없으면 0으로 출력
SELECT
		a.userId,
		a.userName,
		a.userHp,
		sum(a.userPoint) AS `현재포인트`,
		sum(a.userPoint + IFNULL(b.`point`, 0)) AS `총 포인트`
FROM `Users` AS a
...
#문제 4
#모든 주문의 주문번호, 주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오. 
...
#문제 5
#모든 주문의 주문번호, 주문자 아이디, 고객명, 상품명, 주문일자를 조회하시오. 주문번호는 중복없이 상품명은 구분자 ,로 나열할 것
SELECT 
		a.orderNo,
		b.userId,
		b.userName,
...
#문제 8
#아직 상품을 판매하지 않은 판매자의 판매자번호, 판매자상호, 판매자이름, 판매자 연락처를 조회하시오.
SELECT
		a.sellerNo,
		a.sellerBizName,
		a.sellerManager,
...
#문제 9
#모든 주문상세내역 중 개별 상품 가격과 개수에 맞는 총합과 각 할인율을 적용한 최종 총합을 구하고
#최종 총합이 10만원 이상 그리고 큰 금액 순으로 주문번호, 최종총합을 조회하시오.  
SELECT `orderNo`,
		SUM(CEIL((`itemPrice` - (`itemPrice` / 100 * `itemDiscount`)) * `itemCount`)) AS `최종 총합`
		FROM `OrderItems`
GROUP BY `orderNo`
HAVING `최종 총합` >= 100000
ORDER BY `최종 총합` DESC;
...
#문제 10
#장보고 고객이 주문했던 모든 상품명을 고객명, 상품명으로 조회하시오. 단 상품명은 중복 안됨. 상품명은 구분자 , 로 나열
SELECT
		a.userName,
