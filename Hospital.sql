...
INSERT INTO `Patients` VALUES ('P102103', 'D102104', 'N102102', '유재석', '721128-1234567', 'M', '부산', '010-6101-7211', '-', '개그맨');
...
INSERT INTO `Treatments` VALUES (1081008, 'D108101', 'P108101', '오른쪽 발목 뼈 골전', NOW());
...
#문제 2
#모든 간호사의 간호사 ID, 이름, 생년월일, 전공의과 번호, 전공의과 이름을 조회하시오
SELECT
		a.nur_id,
		a.nur_name,
		a.nur_birth,
...
#문제 3
#모든 환자의 환자ID, 이름, 주민번호, 휴대폰, 담당의사 이름, 담당 간호사 이름을 조회하시오
SELECT 
		a.pat_id,
		a.pat_name,
		a.pat_jumin,
...
JOIN `Charts` AS b ON a.treat_no = b.treat_no
JOIN `Patients` AS c ON b.pat_id = c.pat_id
...
JOIN `Patients` AS c ON b.pat_id = c.pat_id
JOIN `Doctors` AS d ON c.doc_id = d.doc_id
JOIN `Departments` AS e ON d.dep_no = e.dep_no
WHERE e.dep_name = '외과';
...
JOIN `Doctors` AS d ON c.doc_id = d.doc_id
JOIN `Departments` AS e ON d.dep_no = e.dep_no
WHERE b.chart_contents LIKE '%화상%';
...
WHERE a.nur_name = '김태희';


#문제 10
#가장 많은 환자 수를 담당하는 간호사ID, 간호사 이름, 담당환자수를 조회하시오
SELECT 
		a.nur_id,
		`nur_name`,
		COUNT(*) AS `환자수`
